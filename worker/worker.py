...
import joblib  # type: ignore # Для загрузки модели KNN
...
RABBITMQ_URL = os.getenv("RABBITMQ_URL", "amqp://rabbitmq:rabbitmq@rabbitmq/")
...
async def process_message(message: aio_pika.IncomingMessage):
    async with message.process():
        ...
        # Publish the result back to a response queue
        ...
        if response_queue:
            await message.channel.default_exchange.publish(
                aio_pika.Message(
                    body=json.dumps(result).encode(), correlation_id=correlation_id
                ),
